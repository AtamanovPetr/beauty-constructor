...
const STYLES: Record<string, string> = {
  ...
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  /* ─── Отзывы на мобилках ─── */
  .slides-container { 
    min-height: 400px;           /* чуть выше, чтобы точно уместить текст + точки */
    position: relative;
  }
  .review-text { 
    font-size:1rem; 
  ...
  .slider-dots {
    ...
    gap: 12px;
    margin-top: 0;              /* убираем внешний отступ, позиционируем абсолютно */
  }
  .dot-label {
    width: 12px;
    height: 12px;
    border-radius: 50%;
    background: rgba(176,122,140,0.3);
    cursor: pointer;
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    // ─── Hero-слайдер ──────────────────────────────────
    const heroSliderRaw = payload.heroSlider || "";
      ...
      <div class="container hero-container">
        ...
        <div class="hero-visual-side">
          ...
          <div class="photo-frame frame-main" style="position:static; width:280px; height:350px; transform:none;">
            <div class="photo-slide" style="opacity:1; animation:none; background-image:url('${escapeHtml(payload.logo || "")}');"></div>
          </div>`
          }
        </div>
      </div>
    ...
    <header class="hero-premium">
      <div class="hero-container">
        <div class="hero-visual-side">
          <div class="photo-frame frame-main">
            <div class="photo-slide slide-img-1" style="background-image:url('${escapeHtml(payload.logo || "")}');"></div>
          </div>
        </div>
        <div class="hero-text-side">
    ...
    // ─── Отзывы (премиум с анимацией, free статичные) ──
