...
const STYLES: Record<string, string> = {
  ...
  * { margin:0; padding:0; box-sizing:border-box; }
  body { font-family:'Montserrat',sans-serif; background:linear-gradient(135deg, #fff5f7, #fdf2f4, #fff5f7); color:#4a3f47; overflow-x:hidden; }
  ...
  .hero-premium { position:relative; min-height:100vh; display:flex; align-items:center; justify-content:center; background:linear-gradient(135deg, #fdf2f4, #f9eef2); overflow:hidden; }
  .hero-ticker { position:absolute; top:0; left:0; width:100%; overflow:hidden; opacity:0.08; font-size:2rem; font-weight:900; letter-spacing:0.2em; color:#b07a8c; }
  .ticker-wrapper { display:flex; white-space:nowrap; animation:ticker 20s linear infinite; }
  @keyframes ticker { 0%{transform:translateX(0)} 100%{transform:translateX(-50%)} }
  .hero-container { display:flex; align-items:center; gap:60px; max-width:1200px; padding:0 20px; z-index:1; }
  .hero-text-side { flex:1; }
  .hero-tag-badge { display:inline-block; border:1px solid #b07a8c; color:#b07a8c; padding:6px 16px; border-radius:20px; font-size:0.9rem; margin-bottom:20px; background:rgba(176,122,140,0.05); }
  ...
  .hero-primary-btn { background:#b07a8c; color:#fff; padding:14px 32px; border-radius:30px; font-weight:700; text-decoration:none; transition:background 0.3s, transform 0.2s; }
  .hero-primary-btn:hover { background:#c9879b; transform:translateY(-2px); }
  .hero-secondary-btn { border:1px solid #b07a8c; color:#b07a8c; padding:14px 32px; border-radius:30px; text-decoration:none; font-weight:600; transition:all 0.3s; background:rgba(255,255,255,0.6); backdrop-filter:blur(10px); }
  .hero-secondary-btn:hover { background:rgba(176,122,140,0.1); transform:translateY(-2px); }
  .hero-visual-side { flex:1; display:flex; justify-content:center; position:relative; height:500px; }
  .photo-frame { border-radius:20px; overflow:hidden; position:absolute; box-shadow:0 12px 30px rgba(176,122,140,0.2); background:linear-gradient(135deg, #f8c3d3, #e4b5c6); transition:transform 0.5s ease, opacity 0.5s ease; }
  .frame-main { width:320px; height:420px; top:0; left:0; z-index:2; transform:rotate(-3deg); }
  .frame-sub { width:260px; height:360px; top:60px; left:180px; z-index:1; transform:rotate(5deg); }
  .photo-slide { width:100%; height:100%; background-size:cover; background-position:center; position:absolute; top:0; left:0; opacity:0; animation:slidePhoto 8s infinite; background-color:transparent; }
  ...
  .services-decor-line { width:60px; height:3px; background:linear-gradient(135deg, #f8c3d3, #e4b5c6); margin:0 auto; border-radius:2px; }
  .services-slider-wrapper { position:relative; max-width:1200px; margin:0 auto; }
  .services-carousel { display:flex; overflow-x:auto; scroll-snap-type:x mandatory; scroll-behavior:smooth; gap:24px; padding:0 20px 20px; -webkit-overflow-scrolling:touch; }
  .services-carousel::-webkit-scrollbar { display:none; }
  .service-card { flex:0 0 280px; scroll-snap-align:start; background:white; border-radius:20px; overflow:hidden; box-shadow:0 4px 16px rgba(176,122,140,0.1); border:1px solid rgba(176,122,140,0.15); transition:transform 0.3s, box-shadow 0.3s; }
  ...
  .slider-arrow { position:absolute; top:50%; transform:translateY(-50%); width:44px; height:44px; border-radius:50%; background:white; border:1px solid rgba(176,122,140,0.3); color:#b07a8c; font-size:1.5rem; cursor:pointer; z-index:5; transition:all 0.3s; box-shadow:0 4px 10px rgba(0,0,0,0.05); }
  ...
  .contacts-action-card { background:white; border-radius:20px; padding:30px; flex:1; border:1px solid rgba(176,122,140,0.2); box-shadow:0 8px 20px rgba(176,122,140,0.08); min-width:280px; box-sizing:border-box; }
  ...
  #bookingForm { display:flex; flex-direction:column; gap:15px; margin-top:20px; width:100%; max-width:100%; box-sizing:border-box; }
  #bookingForm input, #bookingForm select, #bookingForm textarea { background:rgba(255,255,255,0.08); border:1px solid #c9a96e; color:#000; padding:12px 14px; border-radius:6px; font-family:'Montserrat',sans-serif; font-size:0.95rem; outline:none; width:100%; box-sizing:border-box; }
  #bookingForm button { background:#c9a96e; color:#111; border:none; padding:14px; font-weight:700; font-size:1rem; border-radius:6px; cursor:pointer; transition:background 0.3s, transform 0.2s; margin-top:10px; letter-spacing:0.5px; width:100%; box-sizing:border-box; }
  ...
  @media (max-width:768px) {
    ...
    .hero-visual-side { position:relative; height:auto; justify-content:center; display:flex; flex-wrap:wrap; gap:15px; }
    .photo-frame { position:relative; top:auto; left:auto; right:auto; bottom:auto; width:45%; height:250px; transform:none; flex:1; min-width:140px; margin:0; }
  ...
  @media (max-width:480px) {
    ...
    #bookingForm input, #bookingForm select, #bookingForm button { font-size:0.9rem; padding:10px 12px; }
  }
  `,
  free: `
  * { margin:0; padding:0; box-sizing:border-box; }
  body { font-family:'Montserrat',sans-serif; background:#fff8fa; color:#4a3f47; overflow-x:hidden; line-height:1.5; }
  header { padding:40px 20px; }
  .hero-premium { min-height:70vh; display:flex; align-items:center; justify-content:center; background:#fff0f3; text-align:center; border-bottom:1px solid #fce4e8; }
  .hero-ticker { display:none; }
  .hero-container { display:flex; flex-direction:column; align-items:center; gap:30px; max-width:800px; margin:0 auto; }
  .hero-text-side { order:1; }
  .hero-tag-badge { display:inline-block; border:1px solid #d49ba7; color:#b07a8c; padding:6px 16px; border-radius:20px; font-size:0.9rem; margin-bottom:20px; background:rgba(255,255,255,0.8); }
  ...
  .hero-primary-btn { background:#b07a8c; color:#fff; padding:14px 32px; border-radius:30px; font-weight:700; text-decoration:none; display:inline-block; transition:0.2s; }
  .hero-primary-btn:hover { background:#c9879b; transform:translateY(-2px); }
  .hero-secondary-btn { border:1px solid #b07a8c; color:#b07a8c; padding:14px 32px; border-radius:30px; text-decoration:none; font-weight:600; background:white; transition:0.2s; }
  .hero-secondary-btn:hover { background:rgba(176,122,140,0.08); transform:translateY(-2px); }
  .hero-visual-side { order:0; width:160px; height:160px; border-radius:50%; overflow:hidden; box-shadow:0 12px 24px rgba(176,122,140,0.15); background:#f8c3d3; transition:0.3s; }
  ...
  .services-decor-line { width:50px; height:3px; background:linear-gradient(135deg, #f8c3d3, #e4b5c6); margin:0 auto; border-radius:2px; }
  .services-slider-wrapper { max-width:1100px; margin:0 auto; }
  .services-carousel { display:flex; overflow-x:auto; gap:24px; padding:0 20px 20px; scroll-snap-type:x mandatory; justify-content:center; }
  .services-carousel::-webkit-scrollbar { display:none; }
  .service-card { flex:0 0 260px; scroll-snap-align:start; background:#fff; border-radius:18px; overflow:hidden; box-shadow:0 4px 14px rgba(0,0,0,0.04); border:1px solid #f2d9e0; transition:transform 0.2s, box-shadow 0.2s; }
  ...
  .reviews-decor-line { width:50px; height:3px; background:linear-gradient(135deg, #f8c3d3, #e4b5c6); margin:0 auto; border-radius:2px; }
  .slider-wrapper { max-width:650px; margin:0 auto; }
  .review-slide { background:#fff; border-radius:18px; padding:24px; margin-bottom:24px; box-shadow:0 4px 14px rgba(0,0,0,0.03); border:1px solid #f2d9e0; text-align:center; }
  ...
  .contacts { background:#fff; padding:70px 0; }
  ...
  .contacts-action-card { background:#fff; border-radius:18px; padding:30px; flex:1; min-width:280px; border:1px solid #f2d9e0; box-shadow:0 8px 20px rgba(176,122,140,0.06); text-align:center; box-sizing:border-box; }
  .action-card-title { font-size:1.5rem; font-weight:700; color:#4a3f47; margin-bottom:10px; }
  .action-card-desc { color:#8b6e7a; margin-bottom:20px; }
  ...
  .footer-bottom { text-align:center; padding:20px 0 0; color:#8b6e7a; font-size:0.9rem; background:#fff; }
  .beauty-branding { text-align:center; padding:12px 0 8px; background:#fdf2f4; border-top:1px solid #f2d9e0; font-size:0.9rem; color:#b07a8c; }
...
export async function PUT(
  ...
  try {
    ...
    if (!isPro) services = services.slice(0, 3);

    // Отзывы
    const reviewsRaw = (reviews || "")
      .split("\n")
      .filter((line: string) => line.trim());
    const reviewItems = reviewsRaw.map((line: string) => {
      const parts = line.split("|").map((p) => p.trim());
    ...
    const heroImages = heroSliderRaw
      .split("|")
      .map((u: string) => u.trim())
      .filter(Boolean)
      .slice(0, 3);
    const hasHeroSlider = heroImages.length > 0;
    let slides: string[] = [];
    if (hasHeroSlider) {
      for (let i = 0; i < 4; i++)
        slides.push(heroImages[i % heroImages.length]);
    }
    ...
    const brandingHtml = !isPro
      ? `<div class="beauty-branding">Создано в <a href="https://beautyconstructor.ru" target="_blank">Beauty Constructor</a></div>`
      : "";
    ...
    <header class="hero-premium fade-in-up">
      ...
      <div class="container hero-container">
        <div class="hero-text-side">
          ...
          <h1 class="hero-main-title">${escapeHtml(title)}</h1>
          <p class="hero-description">${escapeHtml(phrase || "")}</p>
        ...
        <div class="hero-visual-side">
          ...
          <div class="photo-frame frame-sub">
            ...
            <div class="photo-slide slide-img-4"></div>
              ...
              : `
          <div class="photo-frame frame-main" style="position:static; width:280px; height:350px; transform:none;">
            <div class="photo-slide" style="opacity:1; animation:none; background-image:url('${escapeHtml(logo || "")}');"></div>
          </div>`
          }
        </div>
      </div>
    ...
    <header class="hero-premium">
      <div class="hero-container">
        <div class="hero-visual-side">
          <div class="photo-frame frame-main">
            <div class="photo-slide slide-img-1" style="background-image:url('${escapeHtml(logo || "")}');"></div>
          </div>
        </div>
        <div class="hero-text-side">
    ...
    <footer class="contacts fade-in-up" id="contacts">
      <div class="container">
        ...
        <div class="footer-bottom">
          <p>&copy; 2026 ${escapeHtml(title)}. Все права защищены.</p>
          ${brandingHtml}
        </div>
      </div>
    </footer>`;
...
<head>
  ...
  <style>
    ...
    ${
      isPro && hasHeroSlider
        ? `
    .slide-img-1 { background-image:url('${escapeHtml(slides[0] || "")}'); }
    .slide-img-2 { background-image:url('${escapeHtml(slides[1] || "")}'); }
    .slide-img-3 { background-image:url('${escapeHtml(slides[2] || "")}'); }
    .slide-img-4 { background-image:url('${escapeHtml(slides[3] || "")}'); }
    `
        : ""
    }
  </style>
